...
%% initialization
syms x y theta v omega dt gamma real
...
% system funtions
g = [   x - (v/omega)*(sin(theta) - sin(theta+omega*dt))
        y + (v/omega)*(cos(theta) - cos(theta+omega*dt))
        theta + (omega+gamma)*dt                                 ]   % nonlinear system model
...
gb = [ 	x + v*cos(theta + omega*dt)
        y + v*sin(theta + omega*dt)
        theta + (omega+gamma)*dt                	]   % nonlinear system model

% linearization of the functions
Gb = [diff(gb,x) diff(gb,y) diff(gb,theta)]

V = [diff(g,v) diff(g,omega) diff(g,gamma)]

Vb = [diff(gb,v) diff(gb,omega) diff(gb,gamma)]

% V2 = [diff(g2,x) diff(g2,y) diff(g2,theta)]
dt = 1;
