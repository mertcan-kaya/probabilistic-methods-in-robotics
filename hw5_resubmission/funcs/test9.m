...
% landmark
point(1).x = x_array(1,1)+1.2;
point(1).y = y_array(1,1)+0.7;
point(1).s = 'o';

point(2).x = x_array(1,2)+0.8;
point(2).y = y_array(1,2)-0.45;
point(2).s = '^';

point(3).x = x_array(1,3)+0.5;
point(3).y = y_array(1,3)+0.2;
point(3).s = '^';
...
z_t_hat(:,k) = h_t(app,mu_bar(:,:,k+1),m(:,k));
z_err(:,k) = z_t(:,k)-z_t_hat(:,k);

m_err(:,k) = [  x_t(1,k)+z_err(1,k)*cos(x_t(3,k)+z_err(2,k))
                x_t(2,k)+z_err(1,k)*sin(x_t(3,k)+z_err(2,k))
                z_err(3,k)];
            
q(:,k) = (m(1,k)-mu_bar(1,:,k+1))^2+(m(2,k)-mu_bar(2,:,k+1))^2;
        
H_t(:,:,k) = [ -(m(1,k)-mu_bar(1,:,k+1))/sqrt(q(:,k)), -(m(2,k)-mu_bar(2,:,k+1))/sqrt(q(:,k)), 0
                (m(2,k)-mu_bar(2,:,k+1))/q(:,k), -(m(1,k)-mu_bar(1,:,k+1))/q(:,k), -1
                0, 0, 0];

S_t(:,:,k) = H_t(:,:,k)*Sigma_bar(:,:,k+1)*H_t(:,:,k)'+Q_t;
K_t(:,:,k) = Sigma_bar(:,:,k+1)*H_t(:,:,k)'/S_t(:,:,k);

K_t(:,:,k)*z_err(:,k)
mu(:,:,k+1) = mu_bar(:,:,k+1) + K_t(:,:,k)*z_err(:,k);
Sigma(:,:,k+1) = (eye(3)-K_t(:,:,k)*H_t(:,:,k))*Sigma_bar(:,:,k+1);
...
plot([mu_bar(1,:,k+1) m_hat(1,k)],[mu_bar(2,:,k+1) m_hat(2,k)],':r')
plot(m_hat(1,k),m_hat(2,k),'r','Marker',char(round(z_t(3,k))))
drawEllipse(m_hat(:,k),Q_t,'#FF0000')
plot([x_t(1,k) m_err(1,k)],[x_t(2,k) m_err(2,k)],':c')
